...
class StakePage extends React.Component {
    constructor(props) {
        ...
        this.state = {
            SmartContract: null,
            SmartContractAddr: "0x8F08E7Fce5176F4863Ec54f923b20A04f194dfeC",//[KovanNet]
    ...
    withdraw = async () => {
        ...
        console.log("Withdraw ", pair_list, wei_list, nonce, gasPrice);
    ...
    SetGasSpeed = async (value) => {
        this.setState({
            gwei: (value / 100 * this.props.fastestGasPrice * 621024),
            gasCost: (value / 100 * this.props.fastestGasPrice * 621024 * this.props.USD / Math.pow(10, 9)),
        });
    }
    ...
    render() {    
        ...
        const mapToTokenUnstaking = (data) => {
            return data.map((token, i) => {
                return (<TokenAddr token={token} SmartContract={this.state.SmartContract} WalletAddress={this.props.WalletAddress} key={i} />);
            });
        };
