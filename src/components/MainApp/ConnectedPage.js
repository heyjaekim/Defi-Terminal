...
class ConnectedPage extends React.Component {
    ...
    componentDidMount() {
        ...
        let stakecontract = new this.web3.eth.Contract(this.state.StakeContractABI, this.state.StakeContractAddr);
        let withdrawcontract = new this.web3.eth.Contract(this.state.WithdrawContractABI, this.state.WithdrawContractAddr);
    ...
    Invest = async() => {
        ...
        // Nonce
        const nonce = await this.web3.eth.getTransactionCount(this.state.StakeContractAddr);
        ...
        //                                                                                                                                 console.log(error)
        //                                                                                                                             });;
        await this.state.StakeContract.methods.StakeLPList(pair_list, wei_list, 250, Date.now()+2000)
        .estimateGas({
        ...
        let results = await this.state.StakeContract.methods.StakeLPList(pair_list, wei_list, 250, Date.now()+2000)
        .send({
            from:this.props.WalletAddress, 
    ...
    Withdraw = async() => {
        ...
        if(this.state.percent != 0){
            let balance1 = BigNumber(await this.state.WithdrawContract.methods.GetLPBalance(this.state.pair1_addr).call({from:this.props.WalletAddress}));
        ...
        if(this.state.percent2 != 0){
            let balance2 = BigNumber(await this.state.WithdrawContract.methods.GetLPBalance(this.state.pair2_addr).call({from:this.props.WalletAddress}));
        ...
        if(this.state.percent3 != 0){
            let balance3 = BigNumber(await this.state.WithdrawContract.methods.GetLPBalance(this.state.pair3_addr).call({from:this.props.WalletAddress}));
        ...
        const nonce = await this.web3.eth.getTransactionCount(this.state.WithdrawContractAddr);

        let gasSpeed = 0;
        if(this.props.gasSpeed === "slow"){ gasSpeed = 1.0;}else if(this.props.gasSpeed === "medium"){gasSpeed = 1.2;} else{gasSpeed = 1.5;}

        let gasEstimate = 0;
        await this.state.WithdrawContract.methods.WithdrawLPList(pair_list, wei_list, 300, Date.now()+2000)
        .estimateGas({
            from:this.props.WalletAddress
        })
        .then(function(gasAmount){
        ...
        let results = await this.state.WithdrawContract.methods.WithdrawLPList(pair_list, wei_list, 300,Date.now()+2000)
        .send({
            from:this.props.WalletAddress, 
            value:0,
            gas:Math.floor(gasEstimate * gasSpeed), //gas 량
